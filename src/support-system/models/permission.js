import {save, remove, update, page} from '../services/permissionService';
...
export default {
  ...
  effects: {
    ...
    *save({recordKey}, {select, put}) {
      let {saveRecords, records, pageSize, total} = yield select(state => state.permission);
    ...
    *remove({recordId}, {select, put}) {
      let {pageNumber, pageSize, total, records} = yield select(state => state.permission);
    ...
    *update({recordId}, {select, put}) {
      let {records, editRecords} = yield select(state => state.permission);
      let editRecord = editRecords[recordId];
      let res = yield update({id: editRecord.id, name: editRecord.name, isDisabled: editRecord.isDisabled});
      if (res && res.code === '0' && res.data) {
    ...
    *page(action, {select, put}) {
      let {search, pageNumber, pageSize} = yield select(state => state.permission);
