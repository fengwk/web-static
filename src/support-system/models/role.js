import {save, remove, update, page} from '../services/roleService';
...
export default {
  ...
  effects: {
    ...
    *save({recordKey}, {select, put}) {
      let {saveRecords, records, pageSize, total} = yield select(state => state.role);
    ...
    *remove({recordId}, {select, put}) {
      let {pageNumber, pageSize, total, records} = yield select(state => state.role);
    ...
    *update({recordId}, {select, put}) {
      let {records, editRecords} = yield select(state => state.role);
    ...
    *page(action, {select, put}) {
      let {search, pageNumber, pageSize} = yield select(state => state.role);
